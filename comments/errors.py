...
class NonExistentCaptchaError(CommentException):
    def __init__(self, captcha_id):
        super().__init__(errno=7, status_code=400, err_msg="invalid captcha id %".format(captcha_id))

class NonExistentAnswerError(CommentException):
    def __init__(self):
        super().__init__(errno=10, status_code=400, err_msg="invalid answer - potentially missing")
...
class InvalidPostError(CommentException):
    def __init__(self, post_json):
        super().__init__(errno=9, status_code=400, err_msg="missing/malformed post - are you missing mandatory fields?")
