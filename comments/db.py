...
def is_valid_captcha(sql_cursor, captcha_id):
    ...
    sql_cursor.execute("SELECT EXISTS(SELECT 1 FROM tokenmapping where captcha_id=? AND DATETIME('now') < expiry);", (captcha_id,))
    exists = sql_cursor.fetchone()[0] == 1
    # if stale, remove it from the table (this is a no-op if it didn't exist)
    if not exists:
        sql_cursor.execute("DELETE FROM tokenmapping WHERE captcha_id=?", (captcha_id,))
    return exists

def is_challenge_available(sql_cursor, captcha_id):
    # return whether we can solve for this captcha_id
    sql_cursor.execute("SELECT EXISTS(SELECT 1 from challenges WHERE captcha_id=?)", (captcha_id,))
    return sql_cursor.fetchone()[0] == 1

def get_attempt_count(sql_cursor, captcha_id):
    sql_cursor.execute("SELECT attempts FROM challenges WHERE captcha_id=?", (captcha_id,))
    return sql_cursor.fetchone()[0]

def crossvalidate_answer(sql_cursor, captcha_id, provided_attempt):

    # increment attempt count
    sql_cursor.execute("UPDATE challenges SET attempts = attempts + 1 WHERE captcha_id=?", (captcha_id,))
...
def get_token_hash(sql_cursor, captcha_id):
    sql_cursor.execute("SELECT token FROM tokens WHERE captcha_id=? AND DATETIME('now') < expiry", (captcha_id,))
    row = sql_cursor.fetchone()
    if row is None:
        # delete the token because its expired (no-op if token doesn't exist)
        sql_cursor.execute("DELETE FROM tokens WHERE captcha_id=?", (captcha_id,))
        return None
    return row[0]
...
def store_token(sql_cursor, captcha_id, captcha_token_hash):
    sql_cursor.execute("INSERT INTO tokens(captcha_id, token) VALUES(?,?)", (captcha_id, captcha_token_hash))
    sql_cursor.execute("SELECT expiry FROM tokens WHERE captcha_id=?", (captcha_id,))
    return sql_cursor.fetchone()[0]

def remove_challenge(sql_cursor, captcha_id):
    sql_cursor.execute("DELETE FROM challenges WHERE captcha_id=?", (captcha_id,))

def submit_post(sql_cursor, thread_id, captcha_id, title, author_name, comment_body, email_hash, ip_addr=None):
    # submit post
    sql_cursor.execute("INSERT INTO comments(thread_id, title, author_name, commentbody, emailhash, ipaddr) VALUES(?,?,?,?,?,?);", (thread_id, title, author_name, comment_body, email_hash, ip_addr))
    ...
    sql_cursor.execute("SELECT EXISTS(SELECT 1 from tokens WHERE captcha_id=?);", (captcha_id,))
